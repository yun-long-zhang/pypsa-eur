...
def add_sector_methanol_limit(n, sector, methanol_bus, total_demand, limit_fraction=0.7):
    """
    Add a GlobalConstraint to limit methanol use in a transport sector (shipping or aviation).
    """
    n.add(
        "GlobalConstraint",
        f"{sector}_methanol_limit",
        sense="<=",
        type="primary_energy",
        carrier_attribute="methanol",
        node=methanol_bus,
        constant=limit_fraction * total_demand,
    )
...
def add_biomass(
    ...
    if options["e_bioSNG"]:
        ...
        n.add(
            ...
            #efficiency3=-costs.at["solid biomass", "CO2 intensity"]* costs.at["electrobiofuels", "C in fuel"],
            p_nom_extendable=True,
            capital_cost=costs.at["BioSNG", "capital_cost"] * costs.at["BioSNG", "efficiency"] * 1.3,
            marginal_cost=costs.at["BioSNG", "VOM"] * costs.at["BioSNG", "efficiency"],
        )
...
def add_low_t_industry(n, nodes, industrial_demand, costs, must_run):
    ...
    if options["industry_t"]["low_T"]["methanol"]:
        n.add(
            ...
            bus1=nodes + " lowT industry",
            bus2="co2 atmosphere",
            carrier="methanol for lowT industry",
            p_nom_extendable=True,
            efficiency=1.0,
        ...
        n.add(
            ...
            bus2="co2 atmosphere",
            bus3=spatial.co2.nodes,
            carrier="methanol for lowT industry CC",
            p_nom_extendable=True,
...
def add_aviation(
    ...
    p_set = (
        demand_factor
        * pop_weighted_energy_totals.loc[nodes, all_aviation].sum().sum()
        * 1e6
        / nhours
    ...
    n.add(
        ...
        carrier="kerosene for aviation",
        p_nom_extendable=True,
        #p_nom_max=oil_kerosene_p_nom_max,
        efficiency2=costs.at["oil", "CO2 intensity"],
    )
    ...
    if options["methanol"]["methanol_to_kerosene"]:
        ...
        n.add(
            ...
            / costs.at[tech, "methanol-input"],
            p_nom_extendable=True,
            #p_nom_max=methanol_kerosene_p_nom_max,
            lifetime=costs.at[tech, "lifetime"],
        )
...
def add_shipping(
    ...
    if options["endogenous_shipping"]:
        ...
        #nhours = n.snapshot_weightings.generators.sum()
        #total_shipping_demand = p_set.sum()
        
        # Set max shares for each fuel
        #oil_share_max = 0.2
        #methanol_share_max = 0.7
        #hydrogen_share_max = 0.1
        
        # Compute MW upper bounds for each carrier
        #oil_p_nom_max = oil_share_max * total_shipping_demand  / nhours
        #methanol_p_nom_max = methanol_share_max * total_shipping_demand  / nhours
        #hydrogen_p_nom_max = hydrogen_share_max * total_shipping_demand / nhours
...
if __name__ == "__main__":
    ...
    # --- Add methanol use constraints for shipping and aviation ---

    # For shipping
    try:
        total_shipping_demand = (
            pop_weighted_energy_totals.loc[pop_layout.index, ["total domestic navigation"]].sum().sum()
            + pd.read_csv(snakemake.input.shipping_demand, index_col=0).sum().sum()
        ) * 1e6 / n.snapshot_weightings.generators.sum()
        methanol_bus_shipping = spatial.methanol.shipping if options["methanol"]["regional_methanol_demand"] else ["EU shipping methanol"]
        add_sector_methanol_limit(
            n,
            sector="shipping",
            methanol_bus=methanol_bus_shipping,
            total_demand=total_shipping_demand,
            limit_fraction=0.7
        )
    except Exception as e:
        logger.warning(f"Could not add methanol limit for shipping: {e}")

    # For aviation
    try:
        total_aviation_demand = (
            pop_weighted_energy_totals.loc[pop_layout.index, ["total international aviation", "total domestic aviation"]]
            .sum().sum()
        ) * 1e6 / n.snapshot_weightings.generators.sum()
        methanol_bus_aviation = spatial.oil.aviation if options["regional_oil_demand"] else ["EU aviation"]
        add_sector_methanol_limit(
            n,
            sector="aviation",
            methanol_bus=methanol_bus_aviation,
            total_demand=total_aviation_demand,
            limit_fraction=0.7
        )
    except Exception as e:
        logger.warning(f"Could not add methanol limit for aviation: {e}")
